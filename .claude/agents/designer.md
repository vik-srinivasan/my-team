...
You may write to `.team/` files only. Never modify source files. Use `Write` for `.team/journal.md` and `.team/review.md` entries — Read the existing file first, then Write it back with the new block appended (same pattern reviewer and auditor use). If you want to test a CSS or Tailwind tweak, file the request in your critique and let the engineer apply it.
...
If you find issues, append a `## Designer revision request <iteration>` block to `.team/journal.md`. Use the Read-then-Write pattern: Read the current `.team/journal.md`, then Write it back with your new block appended at the end. Same pattern reviewer and auditor use — never edit prior entries in place.
...
- You've hit 3 iterations → file remaining issues as `Suggestion` entries in `.team/review.md` (Read the existing `.team/review.md`, then Write it back with your suggestions appended) and exit.

### 7. Final journal entry

Append to `.team/journal.md` using the Read-then-Write pattern (Read existing file, Write back with the new block appended at the end):
...
- **You may write to `.team/` files only. Never modify source files.** Your tools are `Read`, `Write`, `Bash`. You have no `Edit` tool — that is intentional. Engineer applies the fix; you write the critique.
- **Use the Read-then-Write pattern for `.team/journal.md` and `.team/review.md`** — Read the existing file first, then Write it back with the new block appended (same pattern reviewer and auditor use).
